...
// Arrays.asList("A type of chemical bond", "A mathematical theorem", "A
// biological process"),
// 3));
...
// assertTrue(Arrays.asList("Math",
// "Geography").contains(result.get(0).getCategory()));
// }
// }
...
// doThrow(new
// RuntimeException("Error")).when(questionRepository).add(any(Question.class));
...
// doThrow(new RuntimeException("Not
// found")).when(questionRepository).update(any(Question.class));
...
// Long id = 99L;
// doThrow(new RuntimeException("Not
// found")).when(questionRepository).deleteById(id);
